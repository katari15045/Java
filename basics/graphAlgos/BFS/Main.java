...
public class Main{
	...
	private static void bfs(ArrayList<Node> nodes, int root){
		...
		while(!queue.isEmpty()){
			curNode = nodes.get(queue.removeFirst());
			System.out.print(curNode.val + ", ");
			curIndex = 0;
			while(curIndex < curNode.edges.size()){
				curEdge = curNode.edges.get(curIndex);
				if(!nodes.get(curEdge.endNodeVal-1).processed){
					queue.add(nodes.get(curEdge.endNodeVal-1).val-1);
					nodes.get(curEdge.endNodeVal-1).processed = true;
				}
				curIndex = curIndex + 1;
			}
		}
